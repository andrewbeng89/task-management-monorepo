...
Ensure the backend task API is described by an OpenAPI 3.1 specification that is served over HTTP and kept in sync with the actual routes, so clients and tooling can discover and rely on the API contract.
