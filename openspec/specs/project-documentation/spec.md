...
Provide a single entry-point root `README.md` so new contributors can understand the task-management app and its monorepo layout, the tech stack, how to set up and run the project locally, the notable engineering/architectural decisions, and the intentional MVP scope limitations — without piecing this together from scattered per-package READMEs and the code itself.
